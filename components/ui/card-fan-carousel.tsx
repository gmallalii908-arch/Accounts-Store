...
const ARROW_CLASSES =
  "relative flex items-center justify-center rounded-full border-[1.5px] border-white/10 bg-white/5 backdrop-blur-[16px] text-white/70 cursor-pointer shrink-0 z-30 outline-none shadow-[0_4px_20px_rgba(0,0,0,0.4)] hover:border-amber-500/40 hover:text-amber-300 active:opacity-70 transition-colors duration-300 before:content-[''] before:absolute before:inset-[3px] before:rounded-full before:border before:border-white/[0.04] before:pointer-events-none";
...
export default function SocialCards({ cards }: SocialCardsProps) {
  ...
  return (
    <section className="flex flex-col items-center w-full py-4 lg:py-8 px-4 md:px-8 relative z-20 overflow-visible min-h-[420px]">
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem] h-[340px] sm:h-[400px]">
          {cards.map((card, index) => {
            const isReviewCard = Boolean(card.name || card.comment);

            const cardInner = isReviewCard ? (
              <div className="relative w-full h-full p-6 flex flex-col justify-between rounded-3xl border border-white/15 bg-surface/90 backdrop-blur-2xl shadow-2xl overflow-hidden group">
                ...
                <div>
                  ...
                  <p className="text-xs leading-relaxed text-fg/90 font-medium line-clamp-4">
                    "{card.comment}"
                  </p>
                </div>
                ...
                <div className="flex items-center gap-3 border-t border-white/10 pt-3.5 mt-auto">
                  <div className="relative">
                    <div className="grid h-9 w-9 shrink-0 place-items-center rounded-2xl bg-gradient-to-tr from-red-600 via-purple-600 to-amber-500 font-black text-white text-xs shadow-lg">
                      {card.name ? card.name.slice(0, 1) : "👤"}
                    </div>
                    <span className="absolute -bottom-1 -left-1 grid h-3.5 w-3.5 place-items-center rounded-full bg-emerald-500 text-[8px] font-bold text-black ring-2 ring-surface">
                      ✓
                    </span>
                  </div>
            ...
            return card.linkUrl ? (
              <a key={index} href={card.linkUrl} target={card.linkUrl.startsWith("http") ? "_blank" : "_self"} rel="noopener noreferrer" className="fan-card absolute w-[260px] h-[340px] sm:w-[300px] sm:h-[380px] cursor-pointer">{cardInner}</a>
            ) : (
              <div key={index} className="fan-card absolute w-[260px] h-[340px] sm:w-[300px] sm:h-[380px]">{cardInner}</div>
            );
          })}
        </div>
      </div>
      ...
      {needsPagination && (
        <div className="flex items-center justify-center gap-4 mt-6 md:mt-8 z-30">
          ...
          <div className="flex items-center gap-2">
            {cards.map((_, i) => (
              <span key={i} className={`w-2 h-2 rounded-full transition-all duration-300 ${i === centerIndex ? "bg-amber-400 scale-[1.3] shadow-[0_0_8px_rgba(251,191,36,0.8)]" : "bg-white/20"}`} />
            ))}
          </div>
